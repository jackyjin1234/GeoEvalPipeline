...
class CLIPAwayAdapter:
    ...
    def _create_temp_dataset(self, item: ImageItem, feature: str, 
        ...
        # We'll process left and right sides separately, then merge
        # For now, let's process the left side
        ...
        target_image = images_dir / f"{item.image_id}_left.jpg"
        target_mask = masks_dir / f"{item.image_id}_left.png"
        
        shutil.copy2(left_image, target_image)
        shutil.copy2(left_mask, target_mask)
    ...
    def _run_clipaway_inference(self, dataset_dir: Path, item: ImageItem, 
        ...
        try:
            ...
            for batch in test_dataloader:
                ...
                logger.debug(f"CLIPAway generated: {result_path}")
                return result_path
            
            raise RuntimeError("No data processed from dataset")
