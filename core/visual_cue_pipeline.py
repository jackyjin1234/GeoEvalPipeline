...
class VisualCuePipeline:
    ...
    def _initialize_components(self):
        ...
        if self.mask_generator is None:
            self.mask_generator = MaskGeneratorAdapter(
                ...
                target_labels=self.config.features.target_labels,
                combine_masks=self.config.features.combine_masks,
                mask_suffix=self.config.features.mask_suffix,
                dilate_masks=self.config.features.dilate_masks,
                kernel_size=self.config.features.kernel_size,
                dilation_iterations=self.config.features.dilation_iterations
            )
    ...
    async def run_pipeline(self) -> Dict[str, Any]:
        ...
        logger.info("Starting visual cue evaluation pipeline")
    ...
    async def _execute_image_selection(self):
        ...
        try:
            ...
            self.state.start_pipeline(len(self.selected_items))
    ...
    async def _execute_mask_generation(self):
        ...
        logger.info(f"Phase 3: Mask Generation ({len(self.selected_items)} items)")
        phase_start = time.time()
        self.state.start_phase(phase, len(self.selected_items))
        ...
        try:
            ...
            for i in range(0, len(self.selected_items), batch_size):
                batch = self.selected_items[i:i + batch_size]
                ...
                # Log progress
                progress_bar = create_progress_bar(completed_items, len(self.selected_items))
                logger.info(f"Mask generation progress: {progress_bar}")
            
            # Validate results
            validation = self.mask_generator.validate_masks(self.selected_items)
            
            # Complete phase
            self.state.complete_phase(phase)
            
            logger.info(f"Generated masks for {completed_items}/{len(self.selected_items)} items")
    ...
    async def _execute_image_processing(self):
        ...
        logger.info(f"Phase 4: Image Processing ({len(self.selected_items)} items)")
        phase_start = time.time()
        self.state.start_phase(phase, len(self.selected_items))
        ...
        try:
            ...
            # Validate results
            validation = self.image_processor.validate_processed_images(self.selected_items)
            
            # Complete phase
            self.state.complete_phase(phase)
            
            logger.info(f"Processed {completed_items}/{len(self.selected_items)} items")
    ...
    async def _execute_clipaway_processing(self):
        ...
        logger.info(f"Phase 5: CLIPAway Processing ({len(self.selected_items)} items)")
        phase_start = time.time()
        self.state.start_phase(phase, len(self.selected_items))
        ...
        try:
            ...
            logger.info(f"CLIPAway processed {completed_items}/{len(self.selected_items)} items")
            logger.info(f"Success rate: {validation['success_rate']:.1f}%")
    ...
    async def _execute_evaluation(self):
        ...
        logger.info(f"Phase 6: Geolocation Evaluation ({len(self.selected_items)} items)")
        phase_start = time.time()
        self.state.start_phase(phase, len(self.selected_items))
        ...
        try:
            ...
            # Calculate completed items (evaluation is all-or-nothing per item)
            ...
            logger.info(f"Items evaluated: {completed_items}/{len(self.selected_items)}")
    ...
    def _calculate_overall_success_rate(self) -> float:
        ...
        completed_phases = sum(1 for phase in self.state.phases.values() if phase.status == PhaseStatus.COMPLETED)
        total_phases = len(PipelinePhase)
